...
class Letter {
 ...
 public:
  Letter();
  Letter(char letterChar, vector<bool> rawArray, int height, int y_baseline, int width, const int xheight, int capheight);
  ~Letter();
 ...
 void createFieldPairs(int nPairs, unsigned char* pairsH, int maxdeltax, int height, int bdepth);
void createLetterPixels(unsigned char* letterPixels);
  ...
  void storeField(int* field, int field_width, int field_height, int field_depth);
  int* field;
  int field_width;
  int field_height;
  int field_depth;
